# This script allows to make arbitrage between Bitstamp and Cex. Each time unit it does:
#     - get Bitstamp and Cex BCH Ticker
...
# - quantity to trade (in euro). must be at least 21 to respect CEX threshold.
# - threshold 
# - threshold that I accept for rebalancing data
# - output file (.R format)


# This script can be run locally doing:
# Rscript Run_Arbitrage_Bitstamp_Cex.R "BCHEUR" 21 0.9 0.5  arbitrage_bitstamp_cex_BCHEUR_7Jun.Rdata
...
# private API for Cex
source("https://raw.githubusercontent.com/ymju86/Cryp-To/master/FUNCTIONS/Private_API_Functions_Cex.R")



# 0 ------ CODES
# This script needs to access you code to the Bitstamp and Cex private APIs.
# You need to create 6 objects for the script to work: key_cex / secret_cex / customer_id_cex / key_bitstamp / secret_bitstamp / customer_id_bitstamp
...
initial_balance_cex=get_my_balance_cex()
init_crypto_plat2 = as.numeric( as.character( initial_balance_cex[ 1, "BCH.available"] ))
init_euro_plat2 = as.numeric( as.character( initial_balance_cex[ 1, "EUR.available"] ))
...
while(TRUE){
  ...
  if( trade_side1==TRUE | trade_side2==TRUE | rebalance_side1==TRUE | rebalance_side2==TRUE ){
    ...
    if( trade_side1==TRUE | rebalance_side1==TRUE){
      ...
      tmp=try( from_euro_to_crypto_cex( euro_to_trade*1.0025, currency) )
      if(is.data.frame(tmp)){ cat("    cex transaction ok\n") }else{ cat("    cex transaction FAILED\n")}
    }
    ...
    if( trade_side2==TRUE | rebalance_side2==TRUE){
      ...
      if(is.data.frame(tmp) & ncol(tmp)==5){ cat("    bitstamp transaction ok\n") ; real_bitstamp=as.numeric(as.character(tmp$price)) }else{ cat("    bitstamp transaction FAILED\n")}
      tmp=try( from_crypto_to_euro_cex( crypto_to_trade, currency) )
      if(is.data.frame(tmp)){ cat("    cex transaction ok\n") }else{ cat("    cex transaction FAILED\n")}
    }
    ...
    tmp=try(get_my_balance_cex())
    if( class(tmp)!="try-error" ){
      current_crypto_plat2 = as.numeric( as.character( tmp$BCH.available ))
      current_euro_plat2 = as.numeric( as.character( tmp$EUR.available ))
      cat("    Cex balance recovered\n")
    }else{ cat("    Cex balance NOT recovered\n") }
